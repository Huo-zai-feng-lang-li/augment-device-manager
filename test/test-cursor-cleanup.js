const { generateStableDeviceId, generateCursorDeviceId } = require('../shared/utils/stable-device-id');
...
async function testCursorCleanup() {
    ...
    try {
        ...
        console.log(`   稳定ID一致性: ${stableId1 === stableId2 ? '✅ 一致' : '❌ 不一致'}`);
        console.log(`   Cursor ID随机性: ${cursorId1 !== cursorId2 ? '✅ 随机' : '❌ 重复'}`);
        console.log(`   两种ID不同: ${stableId1 !== cursorId1 ? '✅ 不同' : '❌ 相同'}\n`);
        ...
        for (const cursorPath of cursorPaths) {
            ...
            const type = exists ? (await fs.stat(cursorPath)).isDirectory() ? '目录' : '文件' : '不存在';
            console.log(`   ${path.basename(cursorPath)}: ${exists ? '✅' : '❌'} ${type}`);
        }
        ...
        const mockStorageData = {
            ...
            "telemetry.devDeviceId": `${newCursorDeviceId.substring(0, 8)}-${newCursorDeviceId.substring(8, 12)}-${newCursorDeviceId.substring(12, 16)}-${newCursorDeviceId.substring(16, 20)}-${newCursorDeviceId.substring(20, 32)}`,
            "telemetry.sqmId": `{${newCursorDeviceId.substring(0, 8).toUpperCase()}-${newCursorDeviceId.substring(8, 12).toUpperCase()}-${newCursorDeviceId.substring(12, 16).toUpperCase()}-${newCursorDeviceId.substring(16, 20).toUpperCase()}-${newCursorDeviceId.substring(20, 32).toUpperCase()}}`
        };
        ...
        const machineIdChanged = resultData["telemetry.machineId"] !== originalData["telemetry.machineId"];
        const deviceIdChanged = resultData["telemetry.devDeviceId"] !== originalData["telemetry.devDeviceId"];
        const otherDataPreserved = resultData["someOtherData"] === originalData["someOtherData"];
        ...
        console.log(`   - Cursor ID随机性: ${cursorId1 !== cursorId2 ? '✅' : '❌'}`);
        console.log(`   - ID类型差异: ${stableId1 !== cursorId1 ? '✅' : '❌'}`);
        console.log(`   - 清理效果验证: ${machineIdChanged && deviceIdChanged ? '✅' : '❌'}`);
...
async function testCompatibility() {
    ...
    try {
        ...
        const activationPreserved = mockActivation.deviceId === stableIdAfterCleanup;
        ...
        console.log(`   兼容性测试: ${activationPreserved && cursorIdChanged ? '✅ 通过' : '❌ 失败'}`);
