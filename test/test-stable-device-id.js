const { generateStableDeviceId, clearDeviceIdCache, hasDeviceIdCache } = require('../shared/utils/stable-device-id');
...
async function testStableDeviceId() {
    ...
    try {
        ...
        console.log(`   是否相同: ${deviceId1 === deviceId2 ? '✅ 是' : '❌ 否'}\n`);
        ...
        console.log(`   与原ID相同: ${deviceId1 === deviceId3 ? '✅ 是' : '❌ 否'}\n`);
        ...
        console.log(`   传统方法稳定性: ${traditionalId1 === traditionalId2 ? '✅ 稳定' : '❌ 不稳定'}`);
        console.log(`   与稳定ID相同: ${deviceId1 === traditionalId1 ? '✅ 是' : '❌ 否'}\n`);
        ...
        console.log(`   与原ID不同: ${deviceId1 !== deviceId4 ? '✅ 是（符合预期）' : '❌ 否（异常）'}\n`);

        console.log('🎉 测试完成！');
        
        // 总结
        console.log('\n📊 测试结果总结:');
        console.log(`   - 设备ID稳定性: ${deviceId1 === deviceId2 && deviceId1 === deviceId3 ? '✅ 通过' : '❌ 失败'}`);
        console.log(`   - 清理操作保护: ${deviceId1 === deviceId3 ? '✅ 通过' : '❌ 失败'}`);
        console.log(`   - 传统方法兼容: ${traditionalId1 === traditionalId2 ? '✅ 通过' : '❌ 失败'}`);
        console.log(`   - 完全重置功能: ${deviceId1 !== deviceId4 ? '✅ 通过' : '❌ 失败'}`);
...
async function testActivationProtection() {
    ...
    try {
        ...
        console.log(`   激活时间: ${new Date(mockActivation.activatedAt).toLocaleString()}`);
        console.log(`   过期时间: ${new Date(mockActivation.expiresAt).toLocaleString()}\n`);
        ...
        console.log(`设备ID保持一致: ${mockActivation.deviceId === deviceIdAfterCleanup ? '✅ 是' : '❌ 否'}`);
