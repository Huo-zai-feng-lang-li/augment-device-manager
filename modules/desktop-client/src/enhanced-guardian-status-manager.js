...
class EnhancedGuardianStatusManager {
  ...
  async getComprehensiveStatus() {
    try {
      ...
      const comprehensiveStatus = {
        ...
        mode: standaloneStatus.isRunning ? 'standalone' : 
              inProcessStatus.isGuarding ? 'inprocess' : 'none',
   ...
   * 分析状态一致性
   */
  analyzeStatusConsistency(standaloneStatus, inProcessStatus, protectionStatus) {
    const issues = [];
    const warnings = [];
    ...
    // 检查防护状态与实际保护的一致性
    const hasActiveGuardian = standaloneStatus.isRunning || inProcessStatus.isGuarding;
