...
class DeviceDetection {
    ...
    async getWindowsSpecificInfo() {
        ...
        try {
            ...
            // 获取硬盘序列号
            const { stdout: disk } = await execAsync('wmic diskdrive get SerialNumber /value');
            ...
            // 获取BIOS信息
            const { stdout: bios } = await execAsync('wmic bios get SerialNumber /value');
    ...
    async getMacOSSpecificInfo() {
        ...
        try {
            // 获取硬件UUID
            const { stdout: uuid } = await execAsync('system_profiler SPHardwareDataType | grep "Hardware UUID"');
            ...
            // 获取序列号
            const { stdout: serial } = await execAsync('system_profiler SPHardwareDataType | grep "Serial Number"');
            ...
            // 获取型号标识符
            const { stdout: model } = await execAsync('system_profiler SPHardwareDataType | grep "Model Identifier"');
    ...
    async getLinuxSpecificInfo() {
        ...
        try {
            // 获取机器ID
            const { stdout: machineId } = await execAsync('cat /etc/machine-id 2>/dev/null || cat /var/lib/dbus/machine-id 2>/dev/null || echo ""');
            if (machineId.trim()) {
                info.machineId = machineId.trim();
            }

            // 获取产品UUID
            const { stdout: uuid } = await execAsync('cat /sys/class/dmi/id/product_uuid 2>/dev/null || echo ""');
            if (uuid.trim()) {
                info.productUUID = uuid.trim();
            }

            // 获取主板序列号
            const { stdout: boardSerial } = await execAsync('cat /sys/class/dmi/id/board_serial 2>/dev/null || echo ""');
            if (boardSerial.trim()) {
                info.boardSerial = boardSerial.trim();
            }

            // 获取系统序列号
            const { stdout: systemSerial } = await execAsync('cat /sys/class/dmi/id/product_serial 2>/dev/null || echo ""');
    ...
    async detectWindowsVM() {
        try {
            ...
            const vmIndicators = ['virtualbox', 'vmware', 'virtual', 'qemu', 'kvm', 'xen'];
    ...
    async detectMacOSVM() {
        try {
            const { stdout } = await execAsync('system_profiler SPHardwareDataType | grep "Model Name"');
            const model = stdout.toLowerCase();
    ...
    async detectLinuxVM() {
        try {
            const { stdout } = await execAsync('dmidecode -s system-product-name 2>/dev/null || echo "unknown"');
            const model = stdout.toLowerCase();
            
            const vmIndicators = ['virtualbox', 'vmware', 'qemu', 'kvm', 'xen', 'bochs'];
            const isVM = vmIndicators.some(indicator => model.includes(indicator));
