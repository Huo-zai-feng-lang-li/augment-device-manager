...
class DeviceManager {
  ...
  async performIDECleanup(ideType, results, options = {}) {
    ...
    try {
      ...
      results.actions.push(`🔍 检测到 ${installedVariants.length} 个${ideConfig.name}变体`);
  ...
  async performIntelligentIDECleanup(ideType, variant, options, results) {
    ...
    await this.cleanAugmentIdentityFiles(results, variant.augmentStorage, variant.name);
  ...
  async updateIDEDeviceIdentity(ideType, variant, results) {
    ...
    if (await fs.pathExists(storageJsonPath)) {
      try {
        ...
        results.actions.push(`🔄 ${variant.name} - 设备ID已更新`);
  ...
  async detectInstalledIDEVariants(ideType) {
    ...
    // 根据IDE类型确定要检测的变体
    const variantNames = ideType === 'cursor' 
      ? ['Cursor'] 
