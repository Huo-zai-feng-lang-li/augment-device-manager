...
async function accurateClientTest() {
  ...
  try {
    ...
    console.log(
      `  清理前登录状态: ${beforeLoginData.isLoggedIn ? "已登录" : "未登录"}`
    );
    ...
    if (beforeLoginData.isLoggedIn) {
      ...
      console.log(
        `  访问令牌: ${beforeLoginData.hasAccessToken ? "存在" : "不存在"}`
      );
      console.log(
        `  刷新令牌: ${beforeLoginData.hasRefreshToken ? "存在" : "不存在"}`
      );
    }
    ...
    if (cleanupResult.actions) {
      const keyActions = cleanupResult.actions.filter(
        (action) =>
    ...
    console.log(
      `  清理后登录状态: ${afterLoginData.isLoggedIn ? "已登录" : "未登录"}`
    );
    ...
    if (afterLoginData.isLoggedIn) {
      ...
      console.log(
        `  访问令牌: ${afterLoginData.hasAccessToken ? "存在" : "不存在"}`
      );
      console.log(
        `  刷新令牌: ${afterLoginData.hasRefreshToken ? "存在" : "不存在"}`
      );
    }
    ...
    console.log(
      `  设备ID更新: ${successRate.deviceIdChanged ? "✅ 成功" : "❌ 失败"}`
    );
    console.log(
      `  摆脱顽固ID: ${successRate.escapedOldId ? "✅ 成功" : "❌ 失败"}`
    );
    console.log(
      `  登录状态保留: ${successRate.loginPreserved ? "✅ 成功" : "❌ 失败"}`
    );
    console.log(
      `  Augment数据清理: ${
        successRate.augmentDataCleared ? "✅ 成功" : "❌ 失败"
      }`
    );
    console.log(
      `  监控机制运行: ${successRate.monitoringWorked ? "✅ 成功" : "❌ 失败"}`
    );
    ...
    console.log(
      `  登录邮箱保留: ${
        beforeLoginData.email === afterLoginData.email ? "✅ 一致" : "❌ 不一致"
      }`
    );
    console.log(
      `  会员状态保留: ${
        beforeLoginData.membershipType === afterLoginData.membershipType
          ? "✅ 一致"
          : "❌ 不一致"
      }`
    );
    ...
    return {
      ...
      recommendation:
        successRate.overall >= 85 ? "production_ready" : "needs_improvement",
    };
  } catch (error) {
...
async function getAccurateLoginData() {
  try {
    ...
    if (await fs.pathExists(storageJsonPath)) {
      ...
      // 检查所有可能的登录标识
      loginData.hasAccessToken = !!(
        data["cursorAuth/accessToken"] &&
        data["cursorAuth/accessToken"].length > 10
      );
      loginData.hasRefreshToken = !!(
        data["cursorAuth/refreshToken"] &&
        data["cursorAuth/refreshToken"].length > 10
      );
...
// 计算精确清理成功率
async function calculateAccurateSuccessRate(
  beforeDeviceId,
  afterDeviceId,
  beforeLoginData,
  afterLoginData,
  ...
  if (beforeLoginData.isLoggedIn && afterLoginData.isLoggedIn) {
    ...
    const tokenPreserved =
      afterLoginData.hasAccessToken || afterLoginData.hasRefreshToken;
  ...
  // 4. Augment数据清理 (10分)
  const augmentCleared =
    cleanupResult.actions &&
    cleanupResult.actions.some(
      (action) =>
  ...
  // 5. 监控机制运行 (5分)
  const monitoringWorked =
    cleanupResult.actions &&
    cleanupResult.actions.some(
      (action) => action.includes("监控") || action.includes("强制更新")
    );
  if (monitoringWorked) {
    score += 5;
...
if (require.main === module) {
  accurateClientTest()
    .then((result) => {
      ...
      console.log(
        `💡 建议: ${
          result.recommendation === "production_ready"
            ? "✅ 可投入生产"
            : "⚠️ 需要改进"
        }`
      );
