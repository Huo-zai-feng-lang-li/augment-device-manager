...
class DeviceManager {
    ...
    getCursorPaths() {
        ...
        if (this.platform === 'win32') {
            ...
            paths.globalStorage = path.join(userHome, 'AppData', 'Roaming', 'Cursor', 'User', 'globalStorage');
            paths.augmentExtension = path.join(paths.extensions, 'augment.vscode-augment-*');
            paths.augmentStorage = path.join(paths.globalStorage, 'augment.vscode-augment');
            paths.stateDb = path.join(paths.globalStorage, 'state.vscdb');
        } else if (this.platform === 'darwin') {
            paths.extensions = path.join(userHome, '.cursor', 'extensions');
            paths.globalStorage = path.join(userHome, 'Library', 'Application Support', 'Cursor', 'User', 'globalStorage');
            paths.augmentExtension = path.join(paths.extensions, 'augment.vscode-augment-*');
            paths.augmentStorage = path.join(paths.globalStorage, 'augment.vscode-augment');
            paths.stateDb = path.join(paths.globalStorage, 'state.vscdb');
        } else {
            // Linux
            paths.extensions = path.join(userHome, '.cursor', 'extensions');
            paths.globalStorage = path.join(userHome, '.config', 'Cursor', 'User', 'globalStorage');
            paths.augmentExtension = path.join(paths.extensions, 'augment.vscode-augment-*');
            paths.augmentStorage = path.join(paths.globalStorage, 'augment.vscode-augment');
            paths.stateDb = path.join(paths.globalStorage, 'state.vscdb');
        }
    ...
    async getAugmentExtensionInfo() {
        try {
            ...
            if (await fs.pathExists(this.cursorPaths.extensions)) {
                ...
                const augmentExt = extensions.find(ext => ext.startsWith('augment.vscode-augment-'));
    ...
    async cleanAugmentStorage(results) {
        try {
            if (await fs.pathExists(this.cursorPaths.augmentStorage)) {
                // 备份原始数据
                const backupPath = this.cursorPaths.augmentStorage + '.backup.' + Date.now();
        ...
        try {
            ...
            for (const tempDir of tempDirs) {
                if (await fs.pathExists(tempDir)) {
                    ...
                    const augmentFiles = files.filter(file => 
    ...
    async resetUsageCount() {
        try {
            ...
            // 创建新的配置文件（如果需要）
            const newConfigPath = path.join(this.cursorPaths.augmentStorage, 'augment-global-state');
            ...
            const basicConfig = {
                ...
                deviceId: require('../../shared/crypto/encryption').generateDeviceFingerprint()
            };
            
            await fs.writeJson(path.join(newConfigPath, 'config.json'), basicConfig, { spaces: 2 });
